...
export default function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const incomplete =
    isOwnProfile && (profile.skills.length === 0 || profile.roles.length === 0);
  ...
  return (
    <div className="pt-8 pb-4">
      ...
      {incomplete && (
        <div className="mb-6 fluted-panel">
          <div className="p-4 flex items-start gap-3 relative z-10">
            <Radar className="h-5 w-5 text-primary shrink-0 mt-0.5" />
            <div className="flex-1">
              <p className="label-harsh text-primary/90">Your profile is incomplete</p>
              <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
                {profile.skills.length === 0 && "No skills listed yet. "}
                {profile.roles.length === 0 && "No roles selected yet. "}
                Teams can&apos;t weigh you as a candidate without them.
              </p>
                ...
                {profile.compat && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold capitalize">{(profile.compat.workStyle ?? "not set").replace("_", " ")}</span>
                    </div>
                    <div className="flex justify-between">
